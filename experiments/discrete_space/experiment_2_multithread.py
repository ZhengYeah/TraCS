...
def process_trajectory(traj):
    location_space, epsilon = load_nyc()[0], 8
...
if __name__ == '__main__':
    ...
    print(error_list)
    print(f"epsilon: 8, tp: {np.mean(error_list[:, 0])}, ngram: {np.mean(error_list[:, 1])}, tracs-d: {np.mean(error_list[:, 2])}, tracs-c: {np.mean(error_list[:, 3])}")